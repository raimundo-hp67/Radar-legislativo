  ...
  return (
    <TableHead className={className}>
      <button
        ...
        className="inline-flex items-center hover:text-stone-900 dark:hover:text-stone-100"
      >
        {children}
...
export function ProjectTable({ projects, onDelete, viewMode = 'basic' }: ProjectTableProps) {
  ...
  const renderBoletinCell = (project: ExtendedProject) => (
    <TableCell className="font-mono text-sm">
      <a
        ...
        className="flex items-center gap-1 text-orange-600 hover:underline dark:text-orange-400"
      >
        {project.boletin}
  ...
  if (viewMode === 'detailed') {
    return (
      <div className="overflow-x-auto rounded-md border dark:border-stone-800">
            ...
            {sortedProjects.map((project) => {
              ...
              return (
                <TableRow key={project.id} className={cn(project.hasRecentChanges && 'bg-yellow-50 dark:bg-yellow-950/20')}>
                  ...
                  <TableCell className="text-sm text-stone-600 dark:text-stone-400">{estado}</TableCell>
                  <TableCell className="max-w-[120px] text-sm text-stone-600 dark:text-stone-400">
                    <span className="line-clamp-2">{project.objetivo || '—'}</span>
                  </TableCell>
                  <TableCell className="max-w-[100px] text-sm text-stone-600 dark:text-stone-400">
                    <span className="line-clamp-2">{project.autores || '—'}</span>
                  </TableCell>
                  <TableCell className="text-sm text-stone-600 dark:text-stone-400">{project.comision || '—'}</TableCell>
                  <TableCell className="text-sm text-stone-600 dark:text-stone-400">{project.dateIngreso || '—'}</TableCell>
                ...
                <TableRow key={project.id} className={cn(project.hasRecentChanges && 'bg-yellow-50 dark:bg-yellow-950/20')}>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {project.linkInformes?.slice(0, 3).map((link, index) => (
                              <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="inline-flex items-center gap-0.5 rounded bg-stone-100 px-1.5 py-0.5 text-xs text-orange-600 hover:bg-stone-200 dark:bg-stone-800 dark:text-orange-400"
                              >
                                {index + 1}
                  ...
                  <TableCell className="text-sm text-stone-600 dark:text-stone-400">
                    <div>
                      ...
                      {lastActionDate && (
                        <span className="mt-0.5 block text-xs text-stone-400">{lastActionDate}</span>
                      )}
                    </div>
                  </TableCell>
                  <TableCell className="text-right">{renderActions(project)}</TableCell>
  ...
  return (
    <div className="overflow-x-auto">
      <Table aria-label="Tabla de proyectos de ley">
        <TableHeader>
          <TableRow className="border-stone-200 bg-stone-50/80 dark:border-stone-800 dark:bg-stone-900/80">
            <SortableHeader field="boletin" className="w-[100px] font-semibold text-stone-700 dark:text-stone-300" sortField={sortField} sortDirection={sortDirection} onSort={handleSort}>Boletín</SortableHeader>
            <SortableHeader field="title" className="max-w-[280px] font-semibold text-stone-700 dark:text-stone-300" sortField={sortField} sortDirection={sortDirection} onSort={handleSort}>Título</SortableHeader>
            <SortableHeader field="relevance" className="w-[90px] font-semibold text-stone-700 dark:text-stone-300" sortField={sortField} sortDirection={sortDirection} onSort={handleSort}>Relevancia</SortableHeader>
            <SortableHeader field="estado" className="w-[120px] font-semibold text-stone-700 dark:text-stone-300" sortField={sortField} sortDirection={sortDirection} onSort={handleSort}>Estado</SortableHeader>
            <TableHead className="w-[100px] font-semibold text-stone-700 dark:text-stone-300">Cámara</TableHead>
            <TableHead className="w-[110px] font-semibold text-stone-700 dark:text-stone-300">Urgencia</TableHead>
            <TableHead className="w-[90px] text-right font-semibold text-stone-700 dark:text-stone-300">Acciones</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {sortedProjects.map((project) => {
            ...
            return (
              ...
              >
                ...
                <TableCell><RelevanceBadge relevance={project.relevance} /></TableCell>
                <TableCell className="text-sm text-stone-600 dark:text-stone-400">{estado}</TableCell>
                <TableCell className="text-sm text-stone-600 dark:text-stone-400">{camara}</TableCell>
                <TableCell className="text-sm text-stone-600 dark:text-stone-400">{urgencia}</TableCell>
                <TableCell className="text-right">{renderActions(project)}</TableCell>
              </TableRow>
            );
          })}
