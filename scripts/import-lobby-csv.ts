/**
 * [LEGACY] Import masivo de audiencias de lobby desde archivos CSV de InfoLobby.
 *
 * ⚠️ Para el uso normal NO necesitas este script: usa el botón "Sincronizar
 * Lobby" de la app o `bun run scripts/sync-lobby.ts`, que bajan las audiencias
 * directo de la API sin archivos intermedios.
 *
 * Este importador es para cargas históricas masivas y requiere CSVs
 * pre-descargados desde https://www.infolobby.cl (sección datos abiertos),
 * convertidos a UTF-8 y dejados en tmp/:
 *   - audiencias_2024.csv         (registros de audiencias, 2024+)
 *   - datosAudiencia_2024.csv     (materia y observaciones por audiencia)
 *   - asistenciasPasivos_utf8.csv (autoridades vinculadas a cada audiencia)
 *   - activos_utf8.csv            (lobbistas vinculados a cada audiencia)
 *
 * ⚠️ DESTRUCTIVO: borra y recrea la tabla lobby_audiencias completa.
 *    Por eso exige el flag --force.
 *
 * Uso: bun run scripts/import-lobby-csv.ts --force
 */
...
if (!process.argv.includes('--force')) {
  console.error('⚠️  Este importador BORRA y recrea la tabla lobby_audiencias completa.');
  console.error('   Para el sync normal usa: bun run scripts/sync-lobby.ts');
  console.error('   Si de verdad quieres el import masivo: bun run scripts/import-lobby-csv.ts --force');
  process.exit(1);
}
